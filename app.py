...
st.title("🗣️ AI Share of Voice (SoV) Анализатор (v4.0 - Авто-Извлечение Брендов)")
...
if st.session_state.step >= 4:
    ...
    st.header("Шаг 4: Определение и Финальный Список Брендов")
    st.info("На основе собранных ответов Gemini автоматически извлеките все упомянутые бренды. Отредактируйте список перед анализом.")
        ...
        if st.session_state.client and st.session_state.raw_responses:
            with st.spinner("LLM анализирует ответы и извлекает бренды..."):
                ...
                system_instruction_extraction = (
                    ...
                    "Извлеките **все** уникальные названия брендов, которые упоминаются в ответах и относятся к индустрии "
                    f"'{st.session_state.industry}', включая бренд '{st.session_state.brand}'. "
                    "Убедитесь, что ваш собственный бренд '{st.session_state.brand}' всегда включен. "
                ...
                if json_extraction_response:
                    try:
                        ...
                        if isinstance(extracted_brands, list) and all(isinstance(b, str) for b in extracted_brands):
                            # Убеждаемся, что наш бренд включен и список уникален
                            unique_brands = list(set([b.strip() for b in extracted_brands if b.strip()]))
                            if st.session_state.brand.strip() not in unique_brands:
                                unique_brands.insert(0, st.session_state.brand.strip())
                                
                            st.session_state.competitors = ", ".join(unique_brands)
                            ...
                            st.session_state.competitors = st.session_state.brand
                    except json.JSONDecodeError:
    ...
    st.session_state.competitors = st.text_area(
        ...
        height=100,
        help="Отредактируйте список, чтобы оставить только те бренды, которые вы хотите включить в анализ SoV. Убедитесь, что ваш бренд включен."
    )
        ...
        if not final_competitors:
            ...
            for i, item in enumerate(st.session_state.raw_responses):
                ...
                system_instruction_analysis = (
                    ...
                    "Ваша задача — определить, какие из брендов из 'СПИСОК_БРЕНДОВ' являются наиболее рекомендуемыми или "
...
if st.session_state.step == 1:
    ...
    st.info("Нажмите 'Предложить Бренды для Отслеживания', чтобы LLM извлек все упомянутые бренды из ответов. Затем отредактируйте список.")
